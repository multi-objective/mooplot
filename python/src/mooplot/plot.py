...
def plot_pf(datasets, type="points", filter_dominated=True, **layout_kwargs):
    ...
    datasets : numpy array
        The `dataset` argument must be Numpy array produced by the :ref:`moocoe:read_datasets()` function - an array with 3-4 columns including the objective data and set numbers.
...
def plot_eaf(
    ...
    **layout_kwargs,
    ...
    type: string or list of strings
        The type argument can be "fill", "points", "lines" to define the plot type (See :func:`plot_datasets` for more details). If multiple datasets are used (dictionary interface) \
